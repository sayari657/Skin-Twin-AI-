"""
Script pour scraper TOUTES les pages Amazon soins du visage
"""
...
class AmazonFullScraper:
    def __init__(self):
        ...
        self.start_url = "https://www.amazon.fr/soins-pour-le-visage-crèmes/b/?ie=UTF8&node=211020031&ref_=sv_beauty_5"
        self.headers = {
        ...
        self.max_pages = 10  # Limite de pages à scraper
    ...
    def extract_product_info(self, product_element):
        ...
        try:
            ...
            description = f"Produit de soin du visage Amazon - {title}"
    ...
    def get_next_page_url(self, soup, current_url):
        ...
        try:
            ...
            next_link = soup.find('a', {'aria-label': 'Suivant'}) or \
                       soup.find('a', {'aria-label': 'Next'}) or \
            ...
            next_links = soup.find_all('a', {'class': 's-pagination-next'})
            if next_links and next_links[0].get('href'):
                return urljoin(self.base_url, next_links[0].get('href'))
                
        except Exception as e:
            print(f"Erreur lors de la recherche de la page suivante: {e}")
    ...
    def scrape_page(self, url, page_num):
        ...
        if not soup:
            print(f"Impossible de recuperer la page {page_num}")
            return []
        ...
        print(f"Trouve {len(product_elements)} produits sur la page {page_num}")
    ...
    def scrape_all_pages(self):
        """Scrape toutes les pages Amazon"""
        print("Debut du scraping complet Amazon soins du visage...")
        print("=" * 60)
        ...
        while current_url and page_num <= self.max_pages:
            ...
            products = self.scrape_page(current_url, page_num)
            all_products.extend(products)
            
            print(f"Page {page_num}: {len(products)} produits extraits")
            ...
            if soup:
                ...
                if next_url and next_url != current_url:
                    ...
                    page_num += 1
                    print(f"Page suivante trouvee: {next_url}")
                    time.sleep(random.uniform(2, 4))  # Pause entre les pages
                else:
                    print("Aucune page suivante trouvee")
                    break
            else:
                print("Impossible de recuperer la page pour trouver la suivante")
                break
        
        print(f"\nScraping termine! {len(all_products)} produits extraits au total")
        return all_products
...
def main():
    """Fonction principale"""
    print("Scraping complet Amazon soins du visage")
    print("=" * 60)
    ...
    print("=" * 60)
    print(f"Scraping complet termine!")
