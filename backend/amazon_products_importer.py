"""
Script pour importer tous les produits Amazon "cosmétique visage"
"""
...
class AmazonProductsImporter:
    def __init__(self):
        self.base_url = "https://www.amazon.fr"
        self.search_url = "https://www.amazon.fr/s?k=cosmétique+visage&__mk_fr_FR=ÅMÅŽÕÑ&crid=ICM9DAK5JTI1&sprefix=cosmétique+visage%2Caps%2C135&ref=nb_sb_noss_1"
        self.headers = {
    ...
    def extract_product_info(self, product_element):
        ...
        try:
            ...
            title_element = product_element.find('h2', {'class': 's-size-mini'}) or \
                          ...
                          product_element.find('span', {'class': 'a-size-medium'})
            ...
            price_element = product_element.find('span', {'class': 'a-price-whole'}) or \
                          product_element.find('span', {'class': 'a-offscreen'})
            price = 0.0
            if price_element:
            ...
            return {
                ...
                'price': price,
                'image': image_url,
                'description': f"Produit cosmétique pour le visage - {title}",
                'category': category,
    ...
    def extract_brand_from_title(self, title):
        ...
        brands = [
            ...
            "Avène", "Eucerin", "Bioderma", "Vichy", "Caudalie", "Sephora",
            "LA PROVENÇALE", "Circle", "L'Oréal", "Maybelline", "Revlon"
        ]
    ...
    def determine_category(self, title):
        ...
        if any(word in title_lower for word in ['crème', 'cream', 'moisturizer']):
            return 'MOISTURIZER'
        elif any(word in title_lower for word in ['sérum', 'serum']):
            return 'SERUM'
        elif any(word in title_lower for word in ['nettoyant', 'cleanser', 'gel']):
            return 'CLEANSER'
        elif any(word in title_lower for word in ['masque', 'mask']):
            return 'MASK'
        elif any(word in title_lower for word in ['tonique', 'toner']):
            return 'TONER'
        elif any(word in title_lower for word in ['exfoliant', 'scrub']):
            return 'EXFOLIANT'
    ...
    def determine_skin_types(self, title):
        ...
        if any(word in title_lower for word in ['grasse', 'oily', 'séborrhée']):
        ...
        if any(word in title_lower for word in ['normale', 'normal']):
    ...
    def determine_target_issues(self, title):
        ...
        if any(word in title_lower for word in ['anti-âge', 'anti-age', 'rides', 'wrinkles', 'jeunesse']):
            issues.append('aging')
        if any(word in title_lower for word in ['acné', 'acne', 'boutons', 'imperfections']):
            issues.append('acne')
        if any(word in title_lower for word in ['taches', 'spots', 'pigmentation']):
        ...
        if not issues:
            issues = ['aging', 'dryness']
        
        return issues
    
    def scrape_products(self):
        """Scrape tous les produits de la page Amazon"""
        print("Debut du scraping des produits Amazon...")
...
def main():
    """Fonction principale"""
    print("Importation des produits Amazon 'cosmetique visage'")
    print("=" * 60)
    
    importer = AmazonProductsImporter()
    
    # Scraper les produits
    products = importer.scrape_products()
    ...
    saved, skipped = importer.save_products_to_database(products)
