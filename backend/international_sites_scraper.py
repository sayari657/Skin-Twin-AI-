#!/usr/bin/env python3
"""
Scraper pour les sites internationaux de cosmétiques
Lookfantastic, Feelunique, Notino
"""
...
def scrape_lookfantastic_page(url):
    """Scrape une page Lookfantastic"""
    print(f"Scraping Lookfantastic: {url}")
    ...
    # Sélecteurs Lookfantastic
    product_cards = soup.find_all('div', class_=['product-item', 'product-tile', 'product-card'])
    ...
    for card in product_cards:
        try:
            ...
            brand = brand_elem.get_text(strip=True) if brand_elem else "Lookfantastic"
            
            if name and name != "Lookfantastic":
                products.append({
                    'name': name,
                    'brand': brand,
                    'price': float(price),
            ...
            print(f"Erreur lors du parsing d'un produit Lookfantastic: {e}")
            continue
    
    print(f"Lookfantastic: {len(products)} produits trouvés")
    return products

def scrape_feelunique_page(url):
    """Scrape une page Feelunique"""
    print(f"Scraping Feelunique: {url}")
    ...
    for card in product_cards:
        try:
            ...
            # Marque
            brand_elem = card.find(['span', 'div'], class_=['brand', 'product-brand'])
            brand = brand_elem.get_text(strip=True) if brand_elem else "Feelunique"
            
            if name and name != "Feelunique":
                products.append({
                    'name': name,
                    'brand': brand,
                    'price': float(price),
            ...
            print(f"Erreur lors du parsing d'un produit Feelunique: {e}")
            continue
    
    print(f"Feelunique: {len(products)} produits trouvés")
    return products

def scrape_notino_page(url):
    """Scrape une page Notino"""
    print(f"Scraping Notino: {url}")
    ...
    for card in product_cards:
        try:
            ...
            # Marque
            brand_elem = card.find(['span', 'div'], class_=['brand', 'product-brand'])
            brand = brand_elem.get_text(strip=True) if brand_elem else "Notino"
            
            if name and name != "Notino":
                products.append({
                    'name': name,
                    'brand': brand,
                    'price': float(price),
            ...
            print(f"Erreur lors du parsing d'un produit Notino: {e}")
            continue
    
    print(f"Notino: {len(products)} produits trouvés")
    return products
...
def generate_international_urls():
    """Génère les URLs pour les sites internationaux"""
    base_urls = {
    ...
    keywords = [
        'face-cream', 'face-serum', 'face-cleanser',
        'face-mask', 'anti-age', 'face-moisturizer',
        'face-exfoliant', 'face-toner', 'skincare'
    ]
    ...
    for site, base_url in base_urls.items():
        for keyword in keywords:
            if site == 'Lookfantastic':
                urls.append(f"{base_url}/search?q={keyword}")
            elif site == 'Feelunique':
                urls.append(f"{base_url}/search?q={keyword}")
            elif site == 'Notino':
                urls.append(f"{base_url}/search?q={keyword}")
...
def scrape_page(url):
    ...
    try:
        if 'lookfantastic.fr' in url:
            return scrape_lookfantastic_page(url)
        elif 'feelunique.com' in url:
            return scrape_feelunique_page(url)
        elif 'notino.fr' in url:
            return scrape_notino_page(url)
        else:
            return []
    except Exception as e:
...
def main():
    """Fonction principale"""
    print("=== SCRAPING SITES INTERNATIONAUX ===")
    print("Sites ciblés: Lookfantastic, Feelunique, Notino")
    
    # Générer les URLs
    urls = generate_international_urls()
