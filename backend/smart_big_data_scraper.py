"""
Système de scraping Big Data intelligent avec contournement des protections
"""
...
class SmartBigDataScraper:
    def __init__(self, max_workers=5):
        ...
        self.sources = {
            'amazon': {
                ...
                'max_pages': 5,  # Réduit pour éviter les blocages
                'delay': (2, 5)  # Délai entre les requêtes
            },
            'pharmacie': {
                'base_url': 'https://www.pharmacie.com',
                'search_terms': [
                    'crème visage', 'sérum visage', 'nettoyant visage', 'masque visage',
    ...
    def extract_pharmacie_products(self, soup, page_url):
        """Extraction Pharmacie.com avec sélecteurs adaptés"""
        products = []
        
        try:
            # Sélecteurs pour Pharmacie.com
            product_elements = (
            ...
            for element in product_elements:
                try:
                    ...
                    title_elem = (
                        ...
                        element.find('div', {'class': 'product-name'}) or
                        element.find('a', {'class': 'product-title'})
                    )
                    if not title_elem:
                        continue
                    ...
                    price_elem = (
                        ...
                        element.find('div', {'class': 'cost'}) or
                        element.find('span', {'class': 'price-current'})
                    )
                    price = 0.0
                    if price_elem:
                        price_text = price_elem.get_text(strip=True).replace('€', '').replace(',', '.')
                    ...
                    if img_elem:
                        ...
                        if image_url and not image_url.startswith('http'):
                            image_url = urljoin(self.sources['pharmacie']['base_url'], image_url)
                    ...
                    product_data = {
                        ...
                        'price': price,
                        'image_url': image_url,
                        'description': f"Produit Pharmacie.com - {title}",
                        'category': self.determine_category(title),
            ...
            print(f"Erreur extraction Pharmacie.com: {e}")
    ...
    def scrape_page_smart(self, url, source_name):
        ...
        try:
            ...
            elif source_name == 'pharmacie':
                return self.extract_pharmacie_products(soup, url)
            else:
                return []
        ...
        for term in source_config['search_terms']:
            for page in range(1, source_config['max_pages'] + 1):
                if source_name == 'amazon':
                    ...
                    # Encodage URL pour Pharmacie.com
                    encoded_term = quote(term, safe='')
    ...
    def run_smart_big_data_scraping(self):
        """Lance le scraping Big Data intelligent"""
        print("=== DÉMARRAGE DU SCRAPING BIG DATA INTELLIGENT ===")
        ...
        print("\n" + "=" * 60)
        print("=== RÉSULTATS DU SCRAPING BIG DATA INTELLIGENT ===")
...
def main():
    """Fonction principale"""
    scraper = SmartBigDataScraper(max_workers=3)  # 3 workers pour éviter les blocages
    scraper.run_smart_big_data_scraping()
